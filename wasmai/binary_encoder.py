...
def encode_reftype(reftype: RefType) -> bytes:
    result = b''
    if reftype.nullable:
        result += bytes([0x6F if isinstance(reftype.heap_type, ValType) and reftype.heap_type == ValType.EXTERNREF else 0x70])
    else:
        result += bytes([0x72 if isinstance(reftype.heap_type, ValType) and reftype.heap_type == ValType.EXTERNREF else 0x73])
...
def encode_instruction(instr: Instruction) -> bytes:
    ...
    if isinstance(instr, ConstInstruction):
        ...
        if instr.block_type is None:
            ...
            result += encode_sleb128(instr.block_type)
        result += encode_vector(instr.then_instructions, encode_instruction)
        if instr.else_instructions:
            result += bytes([0x05])
            result += encode_vector(instr.else_instructions, encode_instruction)
        result += bytes([0x0B])
...
def encode_section(section: Section) -> bytes:
    ...
    if isinstance(section, CustomSection):
        ...
        def encode_func(func):
            func_data = encode_vector(func.locals, lambda l: encode_uleb128(l.count) + encode_valtype(l.val_type))
